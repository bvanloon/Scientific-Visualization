...
void SimulationRealization::solve(int grid_size, fftw_real *vx, fftw_real *vy, fftw_real *vx0, fftw_real *vy0, fftw_real visc, fftw_real dt)
{
   ...
   for (i = 0; i < grid_size; i++)
   {
      ...
      {
         vx0[i + (grid_size + 2) * j] = vx[i + grid_size * j];
         vy0[i + (grid_size + 2) * j] = vy[i + grid_size * j];
      }
   }
   ...
   for (i = 0; i <= grid_size; i += 2)
   {
      ...
      for (j = 0; j < grid_size; j++)
      {
         ...
         U[0] = vx0[i + (grid_size + 2) * j];
         V[0] = vy0[i + (grid_size + 2) * j];
         U[1] = vx0[i + 1 + (grid_size + 2) * j];
         V[1] = vy0[i + 1 + (grid_size + 2) * j];
   ...
   for (i = 0; i < grid_size; i++)
   {
      for (j = 0; j < grid_size; j++)
      {
         vx[i + grid_size * j] = f * vx0[i + (grid_size + 2) * j];
         vy[i + grid_size * j] = f * vy0[i + (grid_size + 2) * j];
      }
   }
}
