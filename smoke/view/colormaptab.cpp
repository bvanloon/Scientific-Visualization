...
{
    position = std::min((float) position, this->ui->clampingMaximumSlider->value());
...
{
    this->ui->clampingMaximumSlider->init(0.0f, 1.0f);
    this->ui->clampingMaximumSlider->setValue(Settings::defaults::visualization::clampEnd);

//    this->ui->clampingMinimumSlider->init(0.0f, 1.0f);
