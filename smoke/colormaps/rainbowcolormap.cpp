...
{
    float g = (6 - 2 * dx) * f + dx;
    int red = fmax(0, (3 - fabs(g - 4) - fabs(g - 5)) / 2.0) * (numColors - 1);
    int green = fmax(0, (4 - fabs(g - 2) - fabs(g - 4)) / 2.0) * (numColors - 1);
    int blue = fmax(0, (3 - fabs(g - 1) - fabs(g - 2)) / 2.0) * (numColors - 1);
