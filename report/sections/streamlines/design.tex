...
Both \t{timeStepMax} and \t{edgeLengthMax} can be set to $\infty$, this makes it possible to discount these factors when computing the streamlines. If both of these values are set to $\infty$ the integration of the streamline only terminates if the magnitude of the vector falls below \magnitudeMin or if \streamLineVertexDisc{i} falls outside of the computational domain.